...
- Add retrieval and answering: top-k vector search over the session's chunks, answers written by Claude and streamed to the client with numbered citations that link to the source section. Below a confidence threshold the bot says it does not know and offers to collect an email address instead.
...
- Add the embeddable widget and the one-line `<script>` tag that mounts it into a shadow root on any third-party page.
...
- `retrieval-answering`: vector search over a session's chunks, Claude-written streamed answers, numbered citations, and the low-confidence handoff to a human.
...
- New external dependencies: Supabase (Postgres with pgvector), the OpenAI embeddings API, the Claude API, and Vercel for hosting, cron and firewall rules.
- New secrets, all PLACEHOLDER until set in `.env`: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`.
